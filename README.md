...
臺灣媒體報導與內容詮釋內容往往都被披上了一層外衣，一事件經過不同媒體的詮釋後，讀者看到的面貌往往截然不同。演算法推薦又加深了「同溫層」效應 — 你接收到的觀點，可能只是整個光譜的一小段。

媒體識讀向來都是一個有挑戰性的社會議題，再開發這項專案之前，我實際做了大量的研究調查。如果一個人能夠看到單個事情的多面角度，可以大幅降低對反面陣營的敵對心理，並且內容基於理性事實角度出發，實際上是有助於回歸了解事件全貌的公民角色，而不是基於顏色出發的跟隨者。

PowerReader 的概念幾乎是站立在「偏見分析」的事實上執行訊息清洗，讓不是自然人的 AI 完成這些事情，並且可以很輕鬆看見整個運算的根據和透明性，也同時讓任何參與公民都是有價值的。

PowerReader 的目標是**讓你看見完整的光譜**：這篇報導偏向哪個陣營？偏了多少？同一件事不同媒體怎麼說？哪些事件只有單一立場的媒體在報導？

與傳統新聞分析工具不同，PowerReader 的分析不是由某個機構或編輯台產出的 — 它由使用者社群用自己的裝置算力集體完成，所有分析邏輯開源、可溯源、可驗證。因為任何組織在同溫層的眼裡都會被貼上特定標籤，這幾乎是不可避免的，從最初的製作就決定不仰賴任何組織支持，我本身也沒有任何背景，所以 PowerReader 的整個開發過程都是由我本人和 Claude 獨立製作完成。

---

## 開發問題

1. PowerReader 這名稱挺挺鳥的，我自己是沒問過 Claude 意見，然後開源軟體也一堆都叫這名稱 XD，搞不好未來有個社群是可以討論改改看。
2. Qwen3 8B 模型是不得已的，我預估是未來會有 Qwen3.5 4B 可以使用，降低門檻，提高品質；LLM 模型需要轉譯成瀏覽器可用，但我本身沒有足夠算力和資源，我只能等 WebGPU 的開源社群提供。
3. 臺灣大語言模型問題同上，根本問題是沒有這麼小的蒸餾的模型，轉譯也是需要大量資源和算力，最首先的是沒有這種臺灣大語言模型在這種小參數能推理。
4. 代碼安全性是可行的，但網域和伺服器都是基於 Cloudflare 免費提供，安全固然有限，GPU 的一切運作必然只會是你開啟網頁才運作，並且不會下載任何檔案。
5. 全網共享這套理念是可以被實踐的，網站只是開頭，基於網站使用情況和人數支持下，會持續開發瀏覽器插件、Line 可相容的新聞閱讀插件等等，或者其它領域。
6. 內容盲測，「事實查核」是可能被統計學實現的，保守成功率落在 60~70% 左右，但最後並沒有實裝，因為現有模型能力/智性不足，成本太高了，因此目前只有做最簡單的偏見和立場分類。
...
- 分析結果由全體使用者集體 GPU 產出，非單一機構判定。關於代理運算，是直接採用 Cloudflare 的 AI 算力進行支持。
