 ...
 *
 * Origin: T07 (src/client/ollama-detect.js)
 * Copied into T04 PWA for standalone deployment on Cloudflare Pages.
 *
 * Usage:
...
export async function checkOllamaStatus(endpoint = OLLAMA_CONFIG.DEFAULT_ENDPOINT) {
  ...
  if (!running) {
    return { ...base, available: false, model_ready: false, error: 'ollama_not_running' };
  }

  const modelReady = await isModelReady(endpoint);
  if (!modelReady) {
    return { ...base, available: true, model_ready: false, error: 'model_not_found' };
  }

  return { ...base, available: true, model_ready: true, error: null };
}
