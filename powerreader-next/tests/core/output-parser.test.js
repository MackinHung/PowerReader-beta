...
describe('parseScoreOutput', () => {
  it('parses normal JSON with bias_score', () => {
    const raw = '{"bias_score": 75}';
  ...
  it('parses JSON wrapped in <think>...</think> blocks', () => {
    const raw = '<think>Let me analyze this...</think>{"bias_score": 30}';
    const result = parseScoreOutput(raw);
...
{"bias_score": 45}`;
  ...
  it('rounds floating-point bias_score with Math.round', () => {
    const raw = '{"bias_score": 75.6}';
    const result = parseScoreOutput(raw);
    expect(result.bias_score).toBe(76);
  });

  it('rounds .5 upward', () => {
    const raw = '{"bias_score": 50.5}';
    const result = parseScoreOutput(raw);
    expect(result.bias_score).toBe(51);
  });

  it('extracts JSON from surrounding text', () => {
    const raw = 'Here is the result: {"bias_score": 60} hope this helps!';
    const result = parseScoreOutput(raw);
...
\`\`\``;
  ...
  it('handles zero scores correctly', () => {
    const raw = '{"bias_score": 0}';
    const result = parseScoreOutput(raw);
    expect(result.bias_score).toBe(0);
  });

  it('handles boundary value 100', () => {
    const raw = '{"bias_score": 100}';
    const result = parseScoreOutput(raw);
    expect(result.bias_score).toBe(100);
  });

  it('uses default for missing bias_score key', () => {
    const raw = '{"is_political": true}';
    const result = parseScoreOutput(raw);
    expect(result.bias_score).toBe(50);
  });
  ...
  it('normalizes camp_ratio when sum exceeds 100', () => {
    const raw = '{"bias_score": 50, "camp_ratio": {"green": 50, "white": 30, "blue": 20, "gray": 50}}';
    const result = parseScoreOutput(raw);
  ...
  it('returns null camp_ratio when all values are zero', () => {
    const raw = '{"bias_score": 50, "camp_ratio": {"green": 0, "white": 0, "blue": 0, "gray": 0}}';
    const result = parseScoreOutput(raw);
    expect(result.camp_ratio).toBeNull();
  });

  it('returns null camp_ratio when a key is missing', () => {
    const raw = '{"bias_score": 50, "camp_ratio": {"green": 50, "white": 30, "blue": 20}}';
    const result = parseScoreOutput(raw);
    expect(result.camp_ratio).toBeNull();
  });

  it('returns null camp_ratio when values are not numbers', () => {
    const raw = '{"bias_score": 50, "camp_ratio": {"green": "high", "white": 30, "blue": 20, "gray": 10}}';
    const result = parseScoreOutput(raw);
    expect(result.camp_ratio).toBeNull();
  });

  it('clamps camp_ratio values to 0-100', () => {
    const raw = '{"bias_score": 50, "camp_ratio": {"green": -10, "white": 50, "blue": 150, "gray": 10}}';
    const result = parseScoreOutput(raw);
  ...
  it('parses is_political=true from output', () => {
    const raw = '{"bias_score": 30, "is_political": true, "emotion_intensity": 40}';
    const result = parseScoreOutput(raw);
    expect(result.is_political).toBe(true);
    expect(result.bias_score).toBe(30);
  });
  ...
  it('parses is_political=false from output', () => {
    const raw = '{"bias_score": 30, "is_political": false, "emotion_intensity": 10}';
    const result = parseScoreOutput(raw);
  ...
  it('defaults is_political to true when missing', () => {
    const raw = '{"bias_score": 70}';
    const result = parseScoreOutput(raw);
    expect(result.is_political).toBe(true);
  });

  it('defaults is_political to true when non-boolean', () => {
    const raw = '{"bias_score": 70, "is_political": "yes"}';
    const result = parseScoreOutput(raw);
    expect(result.is_political).toBe(true);
  });

  it('forces bias_score=50 when is_political=false even if model says otherwise', () => {
    const raw = '{"bias_score": 15, "is_political": false, "emotion_intensity": 5}';
    const result = parseScoreOutput(raw);
    expect(result.bias_score).toBe(50);
  ...
  it('handles v3 output format (missing is_political and emotion_intensity)', () => {
    const raw = '{"bias_score": 65, "camp_ratio": {"green": 20, "white": 30, "blue": 40, "gray": 10}}';
    const result = parseScoreOutput(raw);
...
describe('parseNarrativeOutput', () => {
  ...
  it('returns defaults for null input', () => {
    ...
    expect(result).toEqual({ points: [], key_phrases: [], stances: {} });
  });
  ...
  it('handles <think> blocks before narrative JSON', () => {
    ...
    const result = parseNarrativeOutput(raw);
    expect(result).toEqual({ points: ['p1'], key_phrases: ['k1'], stances: {} });
  });

  it('handles markdown code fences around narrative JSON', () => {
    const raw = '```json\n{"points": ["a", "b"], "key_phrases": ["x"]}\n```';
    const result = parseNarrativeOutput(raw);
    expect(result).toEqual({ points: ['a', 'b'], key_phrases: ['x'], stances: {} });
  });

  it('parses valid stances from JSON output', () => {
    const raw = '{"points": ["p1"], "key_phrases": ["k1"], "stances": {"民進黨": "手法+描述"}}';
    const result = parseNarrativeOutput(raw);
    expect(result.stances).toEqual({ '民進黨': '手法+描述' });
  });

  it('defaults stances to empty object when non-object', () => {
    const raw = '{"points": ["p1"], "key_phrases": ["k1"], "stances": "not an object"}';
    const result = parseNarrativeOutput(raw);
    expect(result.stances).toEqual({});
  });

  it('defaults stances to empty object when missing', () => {
